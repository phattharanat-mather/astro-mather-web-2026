    ...
    <div className="overflow-hidden py-1.5">
      <div className={`flex gap-3 ${reverse ? 'marquee-reverse' : 'marquee'}`} style={{ width: 'max-content' }}>
        {items.map((name, i) => (
          <div
            key={`${name}-${i}`}
            className="shrink-0 px-4 py-2 rounded-xl font-medium whitespace-nowrap"
            style={{
  ...
  return (
    <>
      ...
      <StorytellingLayout
        ...
        padX={false}
        innerClassName="grid grid-cols-1 lg:grid-cols-2 gap-0 items-center"
      >
        {/* Left — narrative */}
        <div className="px-8 py-12">
          <motion.div style={{ opacity: textOpacity, y: textY }}>
            <p
              className="tracking-[0.25em] uppercase mb-4"
